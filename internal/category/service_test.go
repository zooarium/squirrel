...
func TestCategoryService(t *testing.T) {
	...
	t.Run("Multi-app Isolation", func(t *testing.T) {
		// App 1 creates a category
		req1 := CreateCategoryRequest{Name: "App1 Cat", Status: 1}
		cat1, err := svc.Create(ctx, 1, 1, 1, req1)
		assert.NoError(t, err)
		...
		assert.True(t, found, "App 1 should see its own category")

		// App 1 can get it by ID
		catById, err := svc.GetByID(ctx, 1, cat1.ID)
		assert.NoError(t, err)
		...
		// App 2 should NOT get App 1's category by ID
		_, err = svc.GetByID(ctx, 2, cat1.ID)
		assert.ErrorIs(t, err, ErrCategoryNotFound)

		// App 1 can update its category
		updateReq := UpdateCategoryRequest{Name: "Updated", Status: 1}
		updated, err := svc.Update(ctx, 1, cat1.ID, updateReq)
		assert.NoError(t, err)
		assert.Equal(t, "Updated", updated.Name)

		// App 1 can delete its category
		err = svc.Delete(ctx, 1, cat1.ID)
		assert.NoError(t, err)
	})
}
...
func BenchmarkListCategories(b *testing.B) {
	...
	// Seed some data
	for i := 0; i < 10; i++ {
		_, _ = svc.Create(ctx, 1, 1, 1, CreateCategoryRequest{
			Name:   "Category",
			Status: 1,
		})
	}
